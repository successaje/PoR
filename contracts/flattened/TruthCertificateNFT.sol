...
library Bytes {
    ...
    function lastIndexOf(
        ...
        unchecked {
            uint256 length = buffer.length;
            for (
                uint256 i = Math.min(Math.saturatingAdd(pos, 1), length);
                i > 0;
                --i
            ) {
    ...
    function replace(
        ...
        length = Math.min(
            length,
            Math.min(replacement.length - offset, buffer.length - pos)
        );

        // replace
        assembly ("memory-safe") {
            mcopy(
                add(add(buffer, 0x20), pos),
                add(add(replacement, 0x20), offset),
                length
            )
        }
    ...
    function reverseBytes8(bytes8 value) internal pure returns (bytes8) {
        value =
            ...
            ((value & 0x00FF00FF00FF00FF) << 8); // swap bytes
        value =
            ((value & 0xFFFF0000FFFF0000) >> 16) |
            ((value & 0x0000FFFF0000FFFF) << 16); // swap 2-byte long pairs
...
library Strings {
     ...
     * assembly block as such would prevent some optimizations.
     */
    function _unsafeWriteBytesOffset(
        bytes memory buffer,
        uint256 offset,
        bytes1 value
    ) private pure {
...
contract TruthCertificateNFT is ERC721, Ownable {
    ...
    ) ERC721("Truth Certificate", "TRUTH") Ownable(initialOwner) {
    ...
    function mintCertificate(
        ...
        require(
            verificationManager.resolvedAssets(assetId),
            "Asset verification not resolved"
        );
    ...
    function applyTruthDecay(uint256 tokenId) public {
        ...
        require(
            block.timestamp > data.verificationTimestamp + data.decayTimer,
            "Decay timer not reached"
        );
